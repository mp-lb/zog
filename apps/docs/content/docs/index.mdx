...
a compatibility check for legacy collection names before repository and index
operations, so an existing `store-metadata` collection is reported before Zog
